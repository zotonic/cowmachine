...
-spec handler_call(#state{}, any(), binary(), atom(), any(), fun())
    -> {ok, cowboy_middleware:env()}.
%% >>>> START -- MW20160825
handler_call(State, HandlerState, RemainingData, _Callback, {reply, Payload}, NextState) ->
    case websocket_send(Payload, State) of
        ok ->
            NextState(State, HandlerState, RemainingData);
        stop ->
            handler_terminate(State, HandlerState, stop);
        Error = {error, _} ->
            handler_terminate(State, HandlerState, Error)
    end;
%% <<<< END -- MW20160825
